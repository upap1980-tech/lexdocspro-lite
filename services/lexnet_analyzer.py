...
class LexNetAnalyzer:
    def __init__(self, ai_service):
        ...
        self.es_holidays = holidays.ES(years=2026)
    ...
    def analizar_notificacion(self, textos, provider='ollama'):
        analisis = self.ai_service.generar_documento(
            prompt="Analiza esta notificación LexNET y extrae: tipo_documento, cliente, numero_expediente, fecha_recepcion, plazo_respuesta_dias. Devuelve JSON.",
            data=textos, provider=provider
        )
        # Ejemplo plazos comunes LEC
        if 'demanda' in analisis.lower():
